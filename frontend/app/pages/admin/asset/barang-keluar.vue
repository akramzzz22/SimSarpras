...
import { PackageMinus, Plus, Search, RefreshCw, Inbox, Loader2, Trash2 } from 'lucide-vue-next'
import { useAdminService, type MutasiBarang } from '~/services/api/admin'
import Pagination from '~/components/ui/pagination.vue'

definePageMeta({ layout: 'admin', middleware: ['auth', 'admin'], title: 'Barang Keluar' })
...
async function load() {
  ...
  error.value = null
  try {
    const res = await admin.mutasi.list({ jenis: 'keluar', per_page: 100 })
    items.value = res.data
  } catch (e: any) {
    error.value = e?.data?.message ?? 'Gagal memuat riwayat barang keluar.'
  } finally {
    loading.value = false
  }
}
...
async function submit() {
  ...
  try {
    ...
    formError.value = e?.data?.message ?? 'Gagal mencatat barang keluar.'
  } finally {
    saving.value = false
  }
}
...
async function remove(m: MutasiBarang) {
  if (!confirm('Yakin ingin menghapus catatan barang keluar ini?')) return
  try {
...
<template>
  <div class="space-y-4">
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
      <div>
        <h2 class="text-sm font-bold text-gray-900">Barang Keluar</h2>
        <p class="text-sm text-gray-500 mt-1">Catat barang yang keluar dari inventaris (dipinjam permanen, dijual, dihapus, dsb).</p>
      </div>
      <button
    ...
    <div v-if="showForm" class="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
      <h3 class="font-semibold text-gray-900 mb-4 flex items-center gap-2">
        <PackageMinus class="w-4 h-4 text-rose-600" /> Catat Barang Keluar
      </h3>
      <form class="grid sm:grid-cols-2 gap-4" @submit.prevent="submit">
        ...
        <div class="sm:col-span-2">
          ...
          <textarea v-model="form.keterangan" rows="2" placeholder="Contoh: Dihapuskan dari inventaris karena rusak total…" class="w-full rounded-lg border border-gray-300 px-3 py-2.5 text-sm outline-none focus:ring-2 focus:ring-red-500" />
        </div>
        <p v-if="formError" class="sm:col-span-2 text-sm text-rose-600">{{ formError }}</p>
        <div class="sm:col-span-2 flex gap-3">
          <button type="submit" :disabled="saving" class="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl bg-rose-600 text-white text-sm font-semibold hover:bg-rose-700 transition disabled:opacity-60">
          ...
          <tbody class="divide-y divide-gray-50">
            <tr v-for="m in pagedFiltered" :key="m.id" class="hover:bg-gray-50/50 transition">
              ...
              <td class="px-5 py-3.5">
                <span class="text-xs px-2 py-1 rounded bg-rose-50 text-rose-700">-{{ m.jumlah }}</span>
              </td>
            ...
            <tr v-if="!filtered.length && !loading">
              <td colspan="5" class="px-5 py-12 text-center text-gray-400">
                ...
                {{ items.length ? 'Tidak ada catatan yang cocok.' : 'Belum ada catatan barang keluar.' }}
              </td>
            </tr>
          </tbody>
        </table>
