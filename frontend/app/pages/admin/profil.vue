...
definePageMeta({ layout: 'admin', middleware: ['auth', 'admin'], title: 'Profil' })
...
const infoRows = computed<{ icon: Component; label: string; value: string }[]>(() => {
  ...
  rows.push({ icon: Mail, label: 'Email', value: u.email ?? '-' })
  if (u.nip) rows.push({ icon: IdCard, label: 'NIP', value: String(u.nip) })
  if (u.nuptk) rows.push({ icon: BadgeCheck, label: 'NUPTK', value: String(u.nuptk) })
  if (u.no_hp) rows.push({ icon: Phone, label: 'No. HP', value: String(u.no_hp) })
  if (u.jenis_kelamin) rows.push({ icon: VenusAndMars, label: 'Jenis Kelamin', value: u.jenis_kelamin === 'L' ? 'Laki-laki' : 'Perempuan' })
...
<template>
  <div class="max-w-2xl mx-auto space-y-4">
    ...
    >
      ...
      <div class="relative">
        <div
          class="w-20 h-20 rounded-full mx-auto flex items-center justify-center text-2xl font-bold text-white overflow-hidden ring-4 ring-blue-100"
        ...
        <div class="flex flex-wrap items-center justify-center gap-1.5 mt-1.5">
          <span
            ...
            :key="r"
            class="inline-flex items-center gap-1 text-xs px-2.5 py-1 rounded bg-blue-50 text-blue-700"
          >
    ...
    >
      <div class="px-5 py-3 flex items-center gap-2" style="border-bottom: 1px solid var(--app-border-light, #E5E7EB);">
        <Briefcase class="w-4 h-4" style="color: #1D4ED8;" />
        <span class="text-xs font-bold uppercase tracking-wide" style="color: #374151;">Informasi Akun</span>
      </div>
      <div class="grid sm:grid-cols-2 divide-y divide-gray-50 sm:divide-y-0">
        <div
          v-for="(row, i) in infoRows"
          :key="row.label"
          class="px-5 py-4 flex items-start gap-3"
          :class="i % 2 === 1 ? 'sm:border-l' : ''"
          :style="i % 2 === 1 ? { borderLeft: '1px solid var(--app-border-light, #E5E7EB)' } : {}"
        >
          <div class="w-9 h-9 rounded-lg shrink-0 flex items-center justify-center" style="background-color: #EFF6FF;">
            <component :is="row.icon" class="w-4 h-4" style="color: #1D4ED8;" />
          </div>
          <div class="min-w-0">
            <div class="text-xs" style="color: var(--app-faint, #9CA3AF);">{{ row.label }}</div>
            <div class="text-sm font-medium break-words" style="color: var(--app-text, #0F172A);">{{ row.value }}</div>
          </div>
        </div>
      </div>
    </div>
